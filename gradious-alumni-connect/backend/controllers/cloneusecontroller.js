    ...
    try {
     ...
     AND (name LIKE ? OR email LIKE ?)`,
        ...
        const [users] = await db.query(
            `SELECT id, name, email, role, course, batch 
     FROM users 
     WHERE is_deleted = FALSE
     AND (name LIKE ? OR email LIKE ?)
...
exports.getMessages = async (req, res) => {
    try {
        ...
        if (connection.length === 0) {
            return res.status(403).json({
                message: "You can only view messages of your connections ❌",
            });
        }
...
exports.getMessageUsers = async (req, res) => {
    try {
        ...
        const [users] = await db.query(
            ...
            -- get only connected users (better than messages join)
            JOIN network_connections nc 
                ON (nc.sender_id = u.id OR nc.receiver_id = u.id)

            WHERE 
                (nc.sender_id = ? OR nc.receiver_id = ?)
                AND u.id != ?
